...
### 12. Curation-Sensitive 2026 Regression Counts Use The Generated Baseline

The 2026 curation batch that added 1-cyanopropargyl exposed a second class of
stale assertions: several table, figure, and slide regression scripts still
pinned current-census values such as molecule totals, top source/facility
counts, chart percentages, and count labels as local literals. Those values are
expected to move during active 2026 database population, so they now live in
`tests/baselines/production_data.json` and are generated by
`scripts/update_data_baseline.py`.

Historical 2021 reproduction assertions remain hard-coded where they protect
audited legacy behavior. Current/2026 curation-sensitive expectations should be
added to the generated baseline rather than patched directly in individual
regression scripts.
